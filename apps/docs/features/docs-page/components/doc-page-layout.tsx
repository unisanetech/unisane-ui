  ...
  return (
    <div className="w-full overflow-x-clip pb-14 @3xl:pb-24">
      <header
        className={
          heroContent
            ? 'mb-10 grid grid-cols-1 items-center gap-6 @2xl:gap-8 @3xl:mb-16 @3xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] @3xl:gap-12'
            : 'mb-10 @3xl:mb-16'
        }
      >
        <div
          className={
            heroContent
              ? 'order-2 flex flex-col justify-center gap-5 py-1 @3xl:order-1 @3xl:gap-6 @3xl:py-3'
              : 'flex flex-col gap-5 @3xl:gap-6'
          }
        >
          {heroEyebrow ? (
            <div className="flex flex-wrap items-center gap-2">{heroEyebrow}</div>
          ) : null}
          <h1 className="text-on-surface max-w-[9ch] text-[2.5rem] leading-[0.92] font-semibold tracking-tight wrap-break-word @2xl:text-[3.25rem] @4xl:text-[4.25rem]">
            {title}
          </h1>
          <Typography
            variant="bodyLarge"
            className="text-on-surface-variant @2xl:text-title-medium max-w-[34ch] leading-relaxed"
          >
            {description}
          </Typography>
        ...
        {heroContent && (
          <Surface
            ...
            rounded="sm"
            className={`order-1 flex items-stretch @3xl:order-2 ${heroMinHeightClass} ${heroOverflowClass} min-w-0`}
          >
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-10 @3xl:gap-14 @5xl:flex-row">
...
export function DocSection({ id, title, description, children }: DocSectionProps) {
  return (
    <section id={id} className="scroll-mt-24">
      <div className="mb-6 space-y-3 @3xl:mb-7">
