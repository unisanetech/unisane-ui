...
const ScrollAreaVerticalExample = () => (
  <ScrollArea className="h-full w-full rounded-sm border border-outline-variant bg-surface">
    <div className="p-4 space-y-4">
      {Array.from({ length: 10 }).map((_, i) => (
        <div key={i} className="flex items-center gap-3 p-3 bg-surface-container rounded-sm">
...
const ScrollAreaHorizontalExample = () => (
  <ScrollArea orientation="horizontal" className="w-full rounded-sm bg-surface">
    <div className="flex gap-4 p-4">
      {Array.from({ length: 8 }).map((_, i) => (
        <div
          key={i}
          className="w-32 h-24 shrink-0 rounded-sm bg-surface-container flex items-center justify-center border border-outline-variant"
        >
