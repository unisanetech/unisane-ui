...
export const checkboxDoc: ComponentDoc = {
  ...
  choosing: {
    ...
    rows: [
      {
        emphasis: 'Unchecked',
        component: <Checkbox readOnly className="pointer-events-none" />,
        rationale: 'Default state indicating an option is not selected.',
        examples: 'Opt-in options, Filters, Preferences',
      },
      {
        emphasis: 'Checked',
        component: <Checkbox defaultChecked readOnly className="pointer-events-none" />,
        rationale: 'Indicates the option is selected and will be applied.',
        examples: 'Selected items, Enabled features, Agreed terms',
      },
      {
        emphasis: 'Indeterminate',
        component: <Checkbox indeterminate readOnly className="pointer-events-none" />,
        rationale:
      ...
      {
        emphasis: 'Error',
        component: <Checkbox invalid readOnly className="pointer-events-none" />,
        rationale: 'Indicates a validation error or required field that needs attention.',
  ...
  // ─── HIERARCHY SECTION ─────────────────────────────────────────────────────
  hierarchy: {
    description: 'Checkboxes can be used standalone or with labels for clear identification.',
    items: [
      {
  ...
  // ─── PLACEMENT SECTION ─────────────────────────────────────────────────────
  placement: {
    description: 'Checkboxes are commonly used in forms, lists, and settings pages.',
    examples: [
      {
        ...
        visual: (
          <div className="border-outline-variant w-full max-w-xs overflow-hidden rounded-sm border">
            {['Buy groceries', 'Walk the dog', 'Finish report'].map((item, i) => (
              <div
                key={i}
                className="border-outline-variant bg-surface flex items-center gap-3 border-b px-4 py-3 last:border-0"
              >
                <Checkbox defaultChecked={i === 0} />
                <span
                  className={`text-body-medium ${i === 0 ? 'text-on-surface-variant line-through' : 'text-on-surface'}`}
                >
                  {item}
                </span>
              </div>
  ...
  props: [
    {
      name: 'label',
      type: 'ReactNode',
      description: 'Rich label content displayed next to the checkbox.',
    },
    {
      name: 'checked',
      type: 'boolean',
    ...
    },
    {
      ...
      type: 'boolean',
      default: 'false',
      description: 'Publishes aria-invalid and displays the checkbox in an invalid state.',
    },
    {
      name: 'size',
      type: '"sm" | "md"',
