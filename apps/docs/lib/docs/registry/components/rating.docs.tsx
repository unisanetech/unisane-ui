    ...
    <div className="bg-surface border-outline-variant relative w-72 overflow-hidden rounded-sm border p-6 shadow-xl">
      ...
      <div className="mb-3 flex items-center gap-1">
        {[1, 2, 3, 4, 5].map((i) => (
          <span
            key={i}
            className={`material-symbols-outlined ${i <= 4 ? 'text-primary' : 'text-outline'}`}
          >
            star
          </span>
        ))}
      ...
      <p className="text-body-medium text-on-surface-variant">Great product! Highly recommend.</p>
    </div>
  </HeroBackground>
);
...
const RatingHalfExample = () => {
  ...
  return <Rating value={value} onValueChange={setValue} allowHalf showValue />;
};
...
export const ratingDoc: ComponentDoc = {
  ...
  slug: 'rating',
  name: 'Rating',
  description: 'Rating allows users to provide feedback by selecting a number of stars.',
  category: 'selection',
  status: 'stable',
  icon: 'star',
  ...
  // ─── CHOOSING SECTION ──────────────────────────────────────────────────────
  choosing: {
    description: 'Rating components come in different configurations for various feedback needs.',
    columns: {
      emphasis: 'Type',
      component: 'Preview',
  ...
  // ─── PLACEMENT SECTION ─────────────────────────────────────────────────────
  placement: {
    description: 'Ratings are used in reviews, feedback forms, and content evaluation.',
    examples: [
      {
        title: 'Basic rating',
  ...
  accessibility: {
    ...
    focus: ['Focus ring visible on individual stars.', 'Hover preview shows potential rating.'],
  },
