...
Use this document for component API, composition, theming-integration, and no-drift
rules. Use `docs/architecture/design-system.md` for tokens, surfaces, motion, and
...
```bash
pnpm --filter @unisane/ui lint
pnpm --filter @unisane/ui validate
pnpm check:quality
```

Quality lane rule:

- `pnpm check:quality` is the canonical package-level release/confidence lane for `unisane-ui`
