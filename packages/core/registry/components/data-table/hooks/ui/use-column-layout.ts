...
import type { Column, ColumnMetaMap, PinPosition } from '@/components/ui/data-table/types';
import { DENSITY_UTILITY_COLUMN_WIDTHS, type Density } from '@/components/ui/data-table/constants/index';
...
export function useColumnLayout<T>({
  ...
  const columnMeta = useMemo<ColumnMetaMap>(() => {
    ...
    sortedVisibleColumns.forEach((col) => {
      ...
      meta[key] = {
        width: columnWidths[key] ?? (typeof col.width === 'number' ? col.width : 150),
      };
    });
  ...
  const { pinnedLeftWidth, pinnedRightWidth } = useMemo(() => {
    ...
    let leftWidth = checkboxWidth + expanderWidth;
    sortedVisibleColumns.forEach((col) => {
  ...
  if (process.env.NODE_ENV !== 'production' && allColumnsHidden) {
    console.warn('DataTable: All columns are hidden. Consider showing at least one column.');
  }
