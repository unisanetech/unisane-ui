...
import { TableColgroup } from '@/components/ui/data-table/components/colgroup';
import { Table, TableContainer } from '@/components/ui/data-table/components/table';
import { CustomScrollbar } from '@/components/ui/data-table/components/custom-scrollbar';
import { StatusAnnouncer } from '@/components/ui/data-table/components/status-announcer';
import { DataTableLayout, StickyZone } from '@/components/ui/data-table/components/layout';
import { DataTableToolbar, type DataTableToolbarProps } from '@/components/ui/data-table/components/toolbar';
...
import { useVirtualizedColumns } from '@/components/ui/data-table/hooks/features/use-virtualized-columns';
import { useKeyboardNavigation } from '@/components/ui/data-table/hooks/ui/use-keyboard-navigation';
import { useCellSelection } from '@/components/ui/data-table/hooks/ui/use-cell-selection';
...
import {
  ...
  useTableUI,
  useGrouping,
} from '@/components/ui/data-table/context';
import { useDataTableRuntime } from '@/components/ui/data-table/context/provider';
import { ensureRowIds } from '@/components/ui/data-table/utils/ensure-row-ids';
import { getNestedValue } from '@/components/ui/data-table/utils/get-nested-value';
import { getTotalPages, clampPage } from '@/components/ui/data-table/utils/pagination';
import { DENSITY_CONFIG, parseCellId, type Density } from '@/components/ui/data-table/constants/index';
import { useI18n } from '@/components/ui/data-table/i18n';

// ─── TOOLBAR PROPS ─────────────────────────────────────────────────────────

export type ToolbarProps<T extends { id: string }> = Omit<
...
export function DataTableInner<T extends { id: string }>({
  ...
  const {
    ...
    onScroll: handleColumnVirtualizerScroll,
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(updateVerticalScrollMode);
  ...
  const updatePinnedShadowState = useCallback((scrollLeft: number) => {
    const root = dataTableRootRef.current;
    const container = tableContainerRef.current;
    if (!root || !container) return;

    const maxScrollLeft = Math.max(0, container.scrollWidth - container.clientWidth);
    root.style.setProperty('--data-table-pin-shadow-left-alpha', scrollLeft > 0.5 ? '0.1' : '0');
    root.style.setProperty(
      '--data-table-pin-shadow-right-alpha',
      maxScrollLeft - scrollLeft > 0.5 ? '0.1' : '0',
    );
  }, []);
  ...
  useEffect(() => {
    ...
    const syncPinnedShadows = () => updatePinnedShadowState(container.scrollLeft);
    syncPinnedShadows();

    const observer = new ResizeObserver(syncPinnedShadows);
    observer.observe(container);
  ...
  const tableStyle = {
    minWidth: totalTableWidth ? `${totalTableWidth}px` : undefined,
    ...(isColumnVirtualized ? getScrollableContainerStyle() : undefined),
  };
  ...
  return (
    <DataTableLayout verticalScroll={config.verticalScroll}>
      ...
      >
        ...
        {toolbarProps && (
          <StickyZone stickyOffset={config.stickyOffset}>
            <DataTableToolbar
              {...toolbarProps}
              selectedCount={selectedRows.size}
