...
import { useTreeData } from '../../../src/hooks/features/use-tree-data';
import { useInlineEditing } from '../../../src/hooks/features/use-inline-editing';
    ...
    it('should validate based on node level', async () => {
      ...
      const validateCell = vi.fn((rowId: string, columnKey: string, value: unknown) => {
        ...
        if (node && node.level === 0 && columnKey === 'role') {
          ...
          if (
            typeof value === 'string' &&
            !value.toLowerCase().includes('executive') &&
            !value.toLowerCase().includes('ceo')
          ) {
