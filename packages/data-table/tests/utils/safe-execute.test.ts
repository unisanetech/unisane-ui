...
} from '../../src/utils/safe-execute';
import { DataTableErrorCode } from '../../src/errors/base';
import { ErrorSeverity } from '../../src/errors/severity';
import type { ErrorHub } from '../../src/errors/error-hub';
...
describe('createSafeFilters', () => {
  ...
  it('should create map of safe filters', () => {
    const columns = [
      {
        key: 'name',
        filterFn: (row: { name: string }, val: unknown) => row.name.includes(val as string),
      },
      {
        key: 'age',
        filterFn: (row: { name: string }, val: unknown) =>
          (row as unknown as { age: number }).age >= (val as number),
      },
    ];
...
describe('createSafeCellRenderer', () => {
  ...
  const createContext = (
    rowId: string,
    value: unknown = 'test',
  ): CellRenderContext<{ id: string; name: string }> => ({
  ...
  it('should return renderer result on success', () => {
    const renderer = (ctx: CellRenderContext<{ id: string; name: string }>) =>
      `Value: ${ctx.value}`;
