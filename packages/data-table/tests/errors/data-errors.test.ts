  ...
  describe('construction', () => {
    it('should create error with single duplicate ID', () => {
      ...
      expect(error.message).toBe(
        'Duplicate row IDs detected: abc123. Each row must have a unique ID.',
      );
    ...
    it('should create error with multiple duplicate IDs', () => {
      ...
      expect(error.message).toBe(
        'Duplicate row IDs detected: id1, id2, id3. Each row must have a unique ID.',
      );
...
describe('InvalidDataFormatError', () => {
  ...
  describe('real world scenarios', () => {
    ...
    it('should describe row structure issue', () => {
      const error = new InvalidDataFormatError("Each row must be an object with an 'id' property", {
        affectedRows: 10,
      });
...
describe('data error codes', () => {
  it('should use unique error codes for each error type', () => {
    ...
    const codes = new Set([duplicate.code, missing.code, invalidFormat.code, fetchError.code]);
