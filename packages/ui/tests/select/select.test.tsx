...
describe('Select', () => {
  ...
  it('supports controlled value and open state', async () => {
    ...
    const rendered = await render(
      <FruitSelect value="apple" open onValueChange={onValueChange} onOpenChange={onOpenChange} />,
    );
    const trigger = getTrigger(rendered.container);
  ...
  it('participates in form submission and forwards foundation refs', async () => {
    ...
    expect(new FormData(rendered.container.querySelector('form') ?? undefined).get('fruit')).toBe(
      'apple',
    );
