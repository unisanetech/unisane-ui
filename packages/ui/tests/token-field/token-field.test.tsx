...
describe('TokenField', () => {
  ...
  it('keeps chip delete controls out of tab order and supports arrow-key chip deletion', async () => {
    ...
    const rendered = await render(<Harness />);
    const input = rendered.container.querySelector('input') as HTMLInputElement | null;
    const firstChip = rendered.container.querySelector(
      '[aria-label="React. Press Delete to remove."]',
    );
    ...
    function Harness() {
      ...
      return (
        <TokenField
          ...
          validateToken={(token) => (token.includes('@') ? null : 'Enter a valid email address.')}
        />
      );
    }
