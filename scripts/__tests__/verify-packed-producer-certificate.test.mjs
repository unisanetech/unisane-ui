...
test('external consumer install is intrinsically offline and rejects source fallbacks', () => {
  ...
  const packedTarballs = [
    'file:../tarballs/unisane-tokens-0.1.1.tgz',
    'file:../tarballs/unisane-ui-0.1.1.tgz',
    'file:../tarballs/unisane-data-table-0.1.1.tgz',
  ].join('\n');
...
test('frozen external consumer lock is exact, portable, and source-owned', () => {
  ...
  assert.match(lock, /file:\.\.\/tarballs\/unisane-ui-0\.1\.1\.tgz/u);
  ...
  assert.throws(
    () =>
      assertFrozenExternalConsumerLock(
        lock.replace(
          'resolution: {tarball: file:../tarballs/unisane-ui-0.1.1.tgz}',
          'resolution: {integrity: sha512-forged, tarball: file:../tarballs/unisane-ui-0.1.1.tgz}',
        ),
      ),
